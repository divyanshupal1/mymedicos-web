...
const TestPage = ({quizData,category}) => {
    ...
    const {
        ...
        handleAnswer,
        submitQuiz
    } = useQuizHandle(user.uid,quiz.qid,category,total,questions)

    if(loading) return <LoadingScreen/>
    if(isSubmitted) return <ResultPage quiz={quiz} answers={answered} score={score}/>

    return (
        <div className='w-full min-h-screen h-full flex items-start justify-start gap-x-2 max-lg:flex-col-reverse'>
            <div className='w-4/12 max-lg:w-full max-lg:h-auto h-full max-h-screen max-lg:min-h-screen sticky max-lg:static top-0 bg-slate-200 rounded-md  flex flex-col gap-y-4 lg:overflow-y-scroll max-lg:overflow-auto max-lg:scroll-hide'>
                ...
                <div className='sticky bottom-0 mt-auto bg-slate-200 bg-opacity-80 backdrop-blur-md rounded-md p-4'>
                    ...
                    <div className='mt-6 w-full'>
                        <Button className="w-full" onClick={submitQuiz} loading={submitting}>Submit Quiz</Button>
...
const useQuizHandle = (uid,qid,category,total,questions) =>{
    ...
    const handleSubmission = () => {
        setSubmitting(true)
        saveQuizProgress(uid,category,qid,{progress:answered,current:currentQuestion,score,submitted:true})
        .then(()=>{
            setIsSubmitted(true)
            setSubmitting(false)
            toast({
    ...
    useEffect(() => {
        let quizProgress = getQuizProgress(uid,category,qid)    
    ...
    useEffect(() => {
        const timeout = setTimeout(() => {
            if(synced==0){
                setSynced(2)
                saveQuizProgress(uid,category,qid,{progress:answered,current:currentQuestion,score,submitted:isSubmitted})
                .then(()=>{
                    setSynced(1)
                }).catch((err)=>{
                    setSynced(0)
    ...
    return {synced,loading,setLoading,isSubmitted,setIsSubmitted,submitting,submitQuiz:handleSubmission,score,answered,currentQuestion,setCurrentQuestion,handleNext,handlePrev,handleAnswer}
}
...
const saveQuizProgress = async (uid,category,quiz_id,data) => {
    const quizRef = doc(db, "QuizProgress",uid,category, quiz_id);
