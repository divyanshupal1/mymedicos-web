...
const TestPage = ({quizData,category}) => {
    ...
    console.log(questions,total,quiz)
    const {user} = useUserStore((state)=>({user:state.user}))
    let quizProgress = getQuizProgress(user.uid,category,quiz.qid)    
    ...
    const handleSubmission = () => {
        setSubmitting(true)
        setSynced(0)
        saveQuizProgress(user.uid,category,quiz.qid,{progress:answered,current:currentQuestion,score,submitted:true})
        .then(()=>{
            setSynced(1)
            setIsSubmitted(true)
            setSubmitting(false)
    ...
    useEffect(() => {
        const timeout = setTimeout(() => {
            if(synced==0){
                setSynced(2)
                saveQuizProgress(user.uid,category,quiz.qid,{progress:answered,current:currentQuestion,score,submitted:isSubmitted})
                .then(()=>{
                    setSynced(1)
                }).catch((err)=>{
                    setSynced(0)
