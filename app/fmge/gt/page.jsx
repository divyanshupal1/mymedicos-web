...
const GTpage = async () => {
  let quizes =[]
  const quizSnapShot = await admin.firestore().collection('Fmge').doc('Weekley').collection('Quiz').where('speciality','==','Exam').get()
  ...
  return (
    <div className='w-full h-auto p-4 max-w-5xl mx-auto '>
      ...
      <QuizList quizes={JSON.stringify(quizes)} category={"gt"} section={'fmge'}/>
    </div>
  )
}
