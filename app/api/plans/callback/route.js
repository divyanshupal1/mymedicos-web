...
export async function POST(req,res){
    ...
    try {
        ...
        return activatePlan(razorpay_order_id,razorpay_payment_id,razorpay_signature);
...
export const activatePlan = async (razorpay_order_id,razorpay_payment_id="",razorpay_signature="") => {
    ...
    const updateOrderDoc = await db.collection("PlansOrders").doc(razorpay_order_id).update(
        {
            status:"success",
            razorpay_payment_id,
            razorpay_signature
    ...
    const userInSubscription = await db.collection("SubscribedUsers").doc(order.user).get();
    if(!userInSubscription.exists){
        await db.collection("SubscribedUsers").doc(order.user).set({
            [order.plan.section]:subscription
        })
        return new Response(JSON.stringify({success:true,message:"Plan activated!"}));
    }
    
    const subscriptionDoc = await db.collection("SubscribedUsers").doc(order.user).update({
        [order.plan.section]:subscription
    })
